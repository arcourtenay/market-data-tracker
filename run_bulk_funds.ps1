...
Start-Process -FilePath $venvPython `
    -ArgumentList "-u", "_bulk_add_funds.py" `
    -WorkingDirectory $backend `
    -WindowStyle Hidden `
    -RedirectStandardOutput "$logs\bulk_funds.out.log" `
    -RedirectStandardError "$logs\bulk_funds.err.log"

Write-Host "Started bulk fund ingestion against production"
